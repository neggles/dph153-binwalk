...
if { [llength $argv] != 4 } {
  puts "usage: $argv0 <hostname> <aichDelay=0:3slot|1:4slot> <demodAvail(Unused)> <threshold>"
  exit
}

set aichDelay      [lindex $argv 1]
set sigA           0;			# Detect signature 0
set threshold      [lindex $argv 3];    # Detection threshold (MS bits)
set scramCodeX     0x000000
set patternA       0x0;	                # Hadamard code = 0
set rachTti        $::PC82x9Layer0::TTIEnum(20MS);  # Tti = 10ms

writeMsg ulCecSetup [Layer0Message_UL_CEC_PRACH_SETUP \
                     $aichDelay $sigA \
