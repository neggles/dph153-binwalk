...
init()
{
    ...
    if [ ! -f $WRKCFG ]; then
        ...
        ALTCFGDIR=/var/ipaccess/config_standby
    ...
    PDIR=`pwd`
    cd $WDIR
    # Use LD_BIND_NOW to ensure binding errors are detected immediately.
    if [ "$OP_MODE" == "OP_DEVELOPER" ]
    then
       LD_BIND_NOW=1 ./$PROG $CMD_LINE_PARAMS > /tmp/$PROG.txt 2>&1 &
    else
       LD_BIND_NOW=1 ./$PROG $CMD_LINE_PARAMS &
    fi
    cd $PDIR
}
