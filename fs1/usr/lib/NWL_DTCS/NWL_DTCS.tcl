###############################################################################
# DTCS package for NWL_DTCS
###############################################################################

package provide NWL_DTCS 1.0 

namespace eval ::NWL_DTCS {
...
proc DTCSSendMessage { fd msg } {
  ...
  # Test PGC - write the data to the console
#  foreach i $msg {
#    puts -nonewline [format "0x%08x " $i]
#  }
  # Test IM1
...
namespace export \
  ...
  DTCSMessage_DLRX_SCHED_SETUP_REQ \
  DTCSMessage_DLRX_SCHED_SETUP_MSG_REQ\
  DTCSMessage_DLRX_SCHED_DELETE_REQ \
  DTCSMessage_DLRX_SCHED_START_REQ \
  DTCSMessage_DLRX_SCHED_STOP_REQ \
  DTCSMessage_DLRX_SCHED_SETUP_RESP \
  DTCSMessage_DLRX_SCHED_DELETE_RESP \
  DTCSMessage_DLRX_SCHED_START_RESP \
  DTCSMessage_DLRX_SCHED_STOP_RESP \
  DTCSMessage_DLRX_DIAG_SETUP_REQ \
  DTCSMessage_DLRX_DIAG_DELETE_REQ \
  DTCSMessage_DLRX_DIAG_START_REQ \
  DTCSMessage_DLRX_DIAG_STOP_REQ \
  DTCSMessage_DLRX_DIAG_SETUP_RESP \
  DTCSMessage_DLRX_DIAG_DELETE_RESP \
  DTCSMessage_DLRX_DIAG_START_RESP \
  DTCSMessage_DLRX_DIAG_STOP_RESP  
...
set   DTCSMessageTypes(DLRX_SCHED_SETUP_REQ)      0x0400
set   DTCSMessageTypes(DLRX_SCHED_SETUP_MSG_REQ)  0x0400
set   DTCSMessageTypes(DLRX_SCHED_SETUP_RESP)     0x0401
set   DTCSMessageTypes(DLRX_SCHED_DELETE_REQ)     0x0402
set   DTCSMessageTypes(DLRX_SCHED_DELETE_RESP)    0x0403
set   DTCSMessageTypes(DLRX_SCHED_START_REQ)      0x0404
set   DTCSMessageTypes(DLRX_SCHED_START_RESP)     0x0405
set   DTCSMessageTypes(DLRX_SCHED_STOP_REQ)       0x0406
set   DTCSMessageTypes(DLRX_SCHED_STOP_RESP)      0x0407

set   DTCSMessageTypes(DLRX_DIAG_SETUP_REQ)      0x0200
set   DTCSMessageTypes(DLRX_DIAG_SETUP_RESP)     0x0201
set   DTCSMessageTypes(DLRX_DIAG_DELETE_REQ)     0x0202
set   DTCSMessageTypes(DLRX_DIAG_DELETE_RESP)    0x0203
set   DTCSMessageTypes(DLRX_DIAG_START_REQ)      0x0204
set   DTCSMessageTypes(DLRX_DIAG_START_RESP)     0x0205
set   DTCSMessageTypes(DLRX_DIAG_STOP_REQ)       0x0206
set   DTCSMessageTypes(DLRX_DIAG_STOP_RESP)      0x0207
...
proc DTCSMessage { msgId communicationsContext msgBytes } {
  ...
  set msg $msgBytes
#  foreach i $msg { puts -nonewline "0x[format %08x $i] " }
#  puts " "
...
proc  DTCSMessage_DLRX_SCHED_SETUP_REQ {context \
                                      schedulingInstance \
                                      portNumber \
                                      repetition \
                                      frameOffset \
  ...
  set msgLen [ expr ([llength $msgBytes]*4)]
  set payloadchar [format "Inner payload length = %d, " $msgLen]
  puts -nonewline $payloadchar
  ...
  return [DTCSMessage $DTCSMessageTypes(DLRX_SCHED_SETUP_REQ) $context $msg]
}

# Added this to allow alternative parameter list for set-up message
proc  DTCSMessage_DLRX_SCHED_SETUP_MSG_REQ {context \
                                      schedulingInstance \
                                      portNumber \
                                      repetition \
				                      dlRxCfgBusMsg } {
  ...
  variable DTCSMessageTypes
  set msg ""
  set frameOffset 0x00
  set transportChannelId 0x00
  set trigger 0x00
  ...
  set msgLen [ expr ([llength $dlRxCfgBusMsg]*4)]
  set payloadchar [format "Inner payload length = %d, " $msgLen]
  puts -nonewline $payloadchar
  ...
  lappend msg [ expr ($trigger & 0xFF) | \
                     ($transportChannelId & 0xFF) << 8 | \
                     ($msgLen & 0xFFFF) << 16]
  set msg [concat $msg $dlRxCfgBusMsg]

  return [DTCSMessage $DTCSMessageTypes(DLRX_SCHED_SETUP_MSG_REQ) $context $msg]
}

proc  DTCSMessage_DLRX_SCHED_DELETE_REQ {context schedulingInstance } {
  ...
  return [DTCSMessage $DTCSMessageTypes(DLRX_SCHED_DELETE_REQ) $context $msg]
}

proc  DTCSMessage_DLRX_SCHED_START_REQ {context  } {

  variable DTCSMessageTypes
  set msg ""

  return [DTCSMessage $DTCSMessageTypes(DLRX_SCHED_START_REQ) $context $msg]
}

proc  DTCSMessage_DLRX_SCHED_STOP_REQ {context  } {

  variable DTCSMessageTypes
  set msg ""

  return [DTCSMessage $DTCSMessageTypes(DLRX_SCHED_STOP_REQ) $context $msg]
}


proc  DTCSMessage_DLRX_DIAG_SETUP_REQ {context \
                                  diagInstance \
                                  diagTag \
                                  reportingPeriod } {
  variable DTCSMessageTypes
  ...
  return [DTCSMessage $DTCSMessageTypes(DLRX_DIAG_SETUP_REQ) $context $msg]
}


proc  DTCSMessage_DLRX_DIAG_DELETE_REQ {context diagInstance diagTag } {
  ...
  return [DTCSMessage $DTCSMessageTypes(DLRX_DIAG_DELETE_REQ) $context $msg]
}

proc  DTCSMessage_DLRX_DIAG_START_REQ {context} {

  variable DTCSMessageTypes
  set msg ""

  return [DTCSMessage $DTCSMessageTypes(DLRX_DIAG_START_REQ) $context $msg]
}

proc  DTCSMessage_DLRX_DIAG_STOP_REQ {context} {

  variable DTCSMessageTypes
  set msg ""

  return [DTCSMessage $DTCSMessageTypes(DLRX_DIAG_STOP_REQ) $context $msg]
}

proc  DTCSMessage_DLRX_SCHED_SETUP_RESP {context errorCode timeStamp} {

  variable DTCSMessageTypes
  return [DTCSMessageResp $DTCSMessageTypes(DLRX_SCHED_SETUP_RESP) \
                          $context $errorCode $timeStamp ""]

}

proc  DTCSMessage_DLRX_SCHED_DELETE_RESP {context schedulingInstance} {
  ...
  return [DTCSMessage $DTCSMessageTypes(DLRX_SCHED_DELETE_RESP) $context $msg]
}

proc  DTCSMessage_DLRX_SCHED_START_RESP {context errorCode timeStamp } {

  variable DTCSMessageTypes
  return [DTCSMessageResp $DTCSMessageTypes(DLRX_SCHED_START_RESP) \
                          $context $errorCode $timeStamp ""]

}

proc  DTCSMessage_DLRX_SCHED_STOP_RESP {context errorCode timeStamp } {

  variable DTCSMessageTypes
  return [DTCSMessageResp $DTCSMessageTypes(DLRX_SCHED_STOP_RESP) \
                          $context $errorCode $timeStamp ""]

}

proc  DTCSMessage_DLRX_DIAG_SETUP_RESP {context errorCode timeStamp } {

  variable DTCSMessageTypes
  return [DTCSMessageResp $DTCSMessageTypes(DLRX_DIAG_SETUP_RESP) \
                          $context $errorCode $timeStamp ""]

}

proc  DTCSMessage_DLRX_DIAG_DELETE_RESP {context errorCode timeStamp } {

  variable DTCSMessageTypes
  return [DTCSMessageResp $DTCSMessageTypes(DLRX_DIAG_DELETE_RESP) \
                          $context $errorCode $timeStamp ""]

}

proc  DTCSMessage_DLRX_DIAG_START_RESP {context errorCode timeStamp } {

  variable DTCSMessageTypes
  return [DTCSMessageResp $DTCSMessageTypes(DLRX_DIAG_START_RESP) \
                          $context $errorCode $timeStamp ""]

}

proc  DTCSMessage_DLRX_DIAG_STOP_RESP {context errorCode timeStamp } {

  variable DTCSMessageTypes
  return [DTCSMessageResp $DTCSMessageTypes(DLRX_DIAG_STOP_RESP) \
                          $context $errorCode $timeStamp ""]
