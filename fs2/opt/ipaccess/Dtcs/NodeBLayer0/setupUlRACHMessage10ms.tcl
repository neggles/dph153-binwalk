...
# TrCHId #0
set trchReset(0)     1
set ttiLog2(0)       $::PC82x9Layer0::TTIEnum(10MS)
...
set ePlusList(0,0)   [list 768 0 0]
set eMinusList(0,0)  [list 432 0 0]

for { set trchId 0 } { $trchId < $numTrch } { incr trchId } {

# TRCH control (frame i)
writeMsg ulSymCtrl($i) [Layer0Message_UL_TRCH_CTRL [expr $trchId + 64] $trchReset($trchId) \
                                               $ttiLog2($trchId) \
                                               $codingType($trchId) \
                                               $crcType($trchId) \
                                               $rmType($trchId) \
...
set rachTti        $::PC82x9Layer0::TTIEnum(10MS);  # Tti = 10ms
...
set frameOffset        0;		# CFN=0 (first TFCI)
set trigger            4;               # 4=>TFCI RACH
set transportChannelId $phyChId;	# Not used
...
set repetition         100;		# 10 frame period
