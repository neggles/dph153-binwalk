###############################################################################
# Layer0 package for PC82x8
###############################################################################

package provide PC82x8Layer0 1.0

namespace eval ::PC82x8Layer0 {
...
proc Layer0Message_UL_PHYSCH_CTRL { physChanId bypass2ndLevel resetFlag \
				    cfn cctrchSize } {

  variable UlSymbolRateCmdEnum

  set val [bitPack 0 $physChanId 5 0]
  set val [bitPack $val $bypass2ndLevel 6 6]
  set val [bitPack $val $resetFlag 7 7]
  set val [bitPack $val $cfn 15 8]
  set val [bitPack $val $cctrchSize 29 16]
  set val [bitPack $val $UlSymbolRateCmdEnum(PHYS_CHAN) 31 31]

  return $val
}

proc Layer0Message_UL_CCTRCH_CTRL { physChanId numTrch cfn } {

  variable UlSymbolRateCmdEnum

  set val [bitPack 0    $physChanId 5 0]
  set val [bitPack $val $cfn        15 8]
  set val [bitPack $val [expr $numTrch - 1] 19 16]
  set val [bitPack $val $UlSymbolRateCmdEnum(CCTRCH_TRCH) 31 31]
...
proc Layer0Message_UL_TRCH_CTRL { trchId trchReset ttiLog2 codingType crcType \
  ...
  set val [bitPack $val $radioEqBits 31 16]
  lappend msg $val
...
proc DecodeLayer0Message_UL_SYM_CTRL { msgBytes decodedArray } {
  ...
  while {[llength $msg] != 0 } {
    ...
    popWord msg val
    set msgType [bitUnpack $val 31 31]
    ...
    if { $msgType == 0 } {
      ...
      PushIndex decoded "CCTRCH"
      DecodeLayer0Message_UL_CCTRCH_CTRL $val msg decoded
      PopIndex decoded
    }
...
proc DecodeLayer0Message_UL_PHYSCH_CTRL { firstWord msgBytes decodedArray } {
  ...
  set decoded($root.bypass2ndLevel) [bitUnpack $firstWord 6 6]
  set decoded($root.resetFlag) [bitUnpack $firstWord 7 7]
  set decoded($root.cfn) [bitUnpack $firstWord 15 8]
  set decoded($root.cctrchSize) [bitUnpack $firstWord 29 16]
}

proc DecodeLayer0Message_UL_CCTRCH_CTRL { firstWord msgBytes decodedArray } {
  ...
  set decoded($root.physChanId) [bitUnpack $firstWord 5 0]
  set decoded($root.cfn) [bitUnpack $firstWord 15 8]
  set decoded($root.numTrch) [bitUnpack $firstWord 19 16]

  for { set i 0 } { $i < [expr $decoded($root.numTrch) + 1] } { incr i } {
    if { [llength $msg] >= 13 } {
      PushIndex decoded "TRCH\[$i\]"
      DecodeLayer0Message_UL_TRCH_CTRL msg decoded
      PopIndex decoded
    } else {
      puts "Error: unsufficient control for TRCH\[$i\]"
    }
  }
}
...
namespace export \
  ...
  Layer0Message_UL_CEC_RACH_DEMOD_SETUP \
...
proc Layer0Message_UL_CEC_PRACH_SETUP { cecInst demodAvail aichDelay \
                                        diversity localMaxFilter detSel \
 				        sigA sigB threshold scramCodeX \
                                        patternA patternB } {
  variable CecCmdEnum
  set msg ""

  set val [bitPack 0 $CecCmdEnum(PRACH_SETUP) 7 0]
  set val [bitPack $val $cecInst 13 8]
  set val [bitPack $val $demodAvail 21 16]
  lappend msg $val

  set val [bitPack 0 $aichDelay 0 0]
  set val [bitPack $val $diversity 1 1]
  set val [bitPack $val $localMaxFilter 2 2]
  set val [bitPack $val $detSel 3 3]
  set val [bitPack $val $sigA 11 8]
  set val [bitPack $val $sigB 15 12]
  ...
  set val [bitPack $val $patternB 31 16]
  lappend msg $val
...
proc DecodeLayer0Message_UL_CEC_PRACH_SETUP { msgBytes decodedArray } {
  ...
  set decoded($root.diversity) [bitUnpack $val 1 1]
  set decoded($root.localMaxFilter) [bitUnpack $val 2 2]
  set decoded($root.detSel) [bitUnpack $val 3 3]
  set decoded($root.sigA) [bitUnpack $val 11 8]
  set decoded($root.sigB) [bitUnpack $val 15 12]
  ...
  set decoded($root.patternB) [bitUnpack $val 31 16]
}

proc Layer0Message_UL_CEC_PRACH_DELETE { cecInst demodAvail } {
...
proc Layer0Message_UL_CEC_RACH_DEMOD_SETUP { \
          chanId demodAvail dpcchSlotFmt \
	  diversity cfn tDelay scramCodeX scramCodeY \
          srchThreshold srchWinOffset localMaxFilter ttiLog2 signatureIdx \
	  fingerMask freqOffset sfn fingerPosList } {
  ...
  set val [bitPack $val $diversity 3 3]
  set val [bitPack $val $hsDpcchEnable 4 4]
  set val [bitPack $val $CecChanTypeEnum(RACH) 7 7]
  set val [bitPack $val $cfn 15 8]
  ...
  set val [bitPack 0 $srchWinOffset 16 0]
  set val [bitPack $val $localMaxFilter 17 17]
  lappend msg $val

  set val [bitPack 0 $ttiLog2 0 0]
  set val [bitPack $val $signatureIdx 7 4]
  set val [bitPack $val $fingerMask 15 8]
  set val [bitPack $val $freqOffset 26 16]
  set val [bitPack $val $sfn 31 28]
  lappend msg $val

  for { set i 0 } { $i < [llength $fingerPosList] } { incr i } {

    if { $i == 0 } {

      # Antenna A
      set val [bitPack 0 [lindex $fingerPosList $i] 7 0]
      ...
      # Interim fingers
      set val [bitPack $val [lindex $fingerPosList $i] \
		   [expr ((($i+1)*8)-1) % 32] [expr ($i*8) % 32]]
...
proc Layer0Message_UL_CEC_DCH_DEMOD_SETUP { \
          chanId demodAvail dpcchSlotFmt \
	  diversity hsDpcchEnable cfn tDelay scramCodeX scramCodeY \
          srchThreshold nCqiTxRep cqiFbCycle srchWinOffset localMaxFilter \
  ...
  set val [bitPack $val $diversity 3 3]
  set val [bitPack $val $hsDpcchEnable 4 4]
  set val [bitPack $val $CecChanTypeEnum(DCH) 7 7]
  set val [bitPack $val $cfn 15 8]
  set val [bitPack $val $tDelay 31 16]
  lappend msg $val
...
proc DecodeLayer0Message_UL_CEC_RACH_DEMOD_SETUP { msgBytes decodedArray } {
  ...
  popWord msg val
  set decoded($root.srchWinOffset) [bitUnpack $val 16 0]
  set decoded($root.localMaxFilter) [bitUnpack $val 17 17]

  popWord msg val
  set decoded($root.ttiLog2) [bitUnpack $val 0 0]
  set decoded($root.signatureIdx) [bitUnpack $val 7 4]
  set decoded($root.fingerMask) [bitUnpack $val 15 8]
  set decoded($root.freqOffset) [bitUnpack $val 26 16]
  set decoded($root.sfn) [bitUnpack $val 31 28]

  set fingerMask $decoded($root.fingerMask)
  set fingerNum  0
  while { $fingerMask } {

    if { [expr $fingerMask & 1] } {

      if { [expr ($fingerNum % 4) == 0] } {
        popWord msg val
      }

      set decoded($root.finger$fingerNum) [bitUnpack $val \
					       [expr (($fingerNum+1)*8)-1 % 32] \
                                               [expr ($fingerNum*8) % 32]]
    }

    set fingerMask [expr $fingerMask>>1]
    incr fingerNum
  }

  # Discard "empty" finger offsets for ant B if necessary
  if { [expr $fingerMask & 0xF0] == 0 } { popWord msg val }
}
...
proc DecodeLayer0Message_UL_CEC_DCH_DEMOD_SETUP { msgBytes decodedArray } {
  ...
  set decoded($root.diversity) [bitUnpack $val 3 3]
  set decoded($root.hsDpcchEnable) [bitUnpack $val 4 4]
  set decoded($root.chanType) [bitUnpack $val 7 7]
  set decoded($root.cfn) [bitUnpack $val 15 8]
  set decoded($root.tDelay) [bitUnpack $val 31 16]

  # Handle RACH demod setup case
  if { $decoded($root.chanType) == 0 } {
    DecodeLayer0Message_UL_CEC_RACH_DEMOD_SETUP msg decoded
  } else {
    popWord msg val
    set decoded($root.scramCodeX) [bitUnpack $val 24 0]
...
proc Layer0Message_UL_CEC_DCH_TFCI { \
                  chanId demodAvail betaC betaD sfLog2 sfIdx \
                  compModePattern  } {

  return [Layer0Message_UL_CEC_DCH_TFCI_SIR $chanId $demodAvail $betaC \
          $betaD $sfLog2 $sfIdx $compModePattern 0]
...
proc Layer0Message_UL_CEC_DCH_TFCI_SIR { \
  ...
  set val [bitPack 0 $betaD 3 0]
  set val [bitPack $val $betaC 7 4]
  set val [bitPack $val $sfLog2 11 8]
  set val [bitPack $val $sfIdx 14 12]
...
proc DecodeLayer0Message_UL_CEC_DCH_TFCI_SIR { msgBytes decodedArray } {
  ...
  popWord msg val
  set decoded($root.betaD) [bitUnpack $val 3 0]
  set decoded($root.betaC) [bitUnpack $val 7 4]
  set decoded($root.sfLog2) [bitUnpack $val 11 8]
  set decoded($root.sfIdx) [bitUnpack $val 14 12]
...
proc Layer0Message_UL_CEC_SIR_SETUP { chanId demodAvail totalNoisePower \
                                      sirTarget deltaSIR1 deltaSIRAfter1 \
                                      deltaSIR2 deltaSIRAfter2 } {
  ...
  set val [bitPack 0 $deltaSIR1 7 0]
  set val [bitPack $val $deltaSIRAfter1 15 8]
  set val [bitPack $val $deltaSIR2 23 16]
  set val [bitPack $val $deltaSIRAfter2 31 24]
  lappend msg $val
